...
import { Camera, Crown, Loader2, MailCheck, MailWarning } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const getInitials = (emailOrId: string | null) => {
    if (!emailOrId) return "..";
    return emailOrId.substring(0, 2).toUpperCase();
  }

  return (
    <div className="container mx-auto flex h-[calc(100vh-8rem)] items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="items-center text-center">
            <div className={`relative group ${!user.isAnonymous && 'cursor-pointer'}`} onClick={handleFileSelect}>
                <Avatar className="h-24 w-24 mb-4">
                    <AvatarImage src={user.photoURL ?? ''} alt="User avatar" />
                    <AvatarFallback>{getInitials(user.email || user.uid)}</AvatarFallback>
                </Avatar>
                {!user.isAnonymous && (
                    <div
          ...
          <CardDescription className="flex items-center gap-2 mt-2">
            {user.isAnonymous ? (
                <Badge variant="outline">Usuario Anónimo</Badge>
            ) : (
                <>
                {user.email}
                {user.emailVerified ? (
        ...
        {user.isAnonymous && (
            <div className="space-y-3">
                <p className="text-sm text-muted-foreground">
                    ¡Convierte tu cuenta para desbloquear más funciones!
                </p>
                <Button asChild>
                    <Link href="/upgrade">
                        <Crown className="mr-2 h-4 w-4" />
                        Conviértete en Permanente
                    </Link>
                </Button>
            </div>
        )}
